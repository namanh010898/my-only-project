...
namespace Daihoc_FPT_News.Controllers
{
    ...
    {
        ISubcribeRepository repository; 
        public SubcribeController(ICacheHelper cacheHelper, ISubcribeRepository _repository) : base(cacheHelper)
        { 
        ...
        { 
            try
            {
                ...
                var listSubcribe = await repository.List();

                // return 404 nếu menu null
                if (listSubcribe == null || listSubcribe.Count == 0)
                {
                    return NotFound(); 
                }

                ViewBag.listSubcribe = listSubcribe;
                ViewBag.SubcribeCount = listSubcribe.Count;
                return View();
        ...
        [HttpGet]
        [Route("add/subcribe")]
        public async Task<IActionResult> AddSubcribe()
        {
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] Subcribe model)
        {
            if (ModelState.IsValid)
            {
                ...
                clsMail.Send(model.Name, model.Description);
        ...
        [HttpPost]
        [Route("api/edit")]
        public async Task<IActionResult> Update([FromBody] Subcribe model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] Subcribe model)
        {
            if (ModelState.IsValid)
            {
                try
